...
public class ButtonFileBrowserNormal : ButtonCustom
{
    ...
    public override void OnPointerDown(PointerEventData eventData)
    {
        if (catalog.currentType == 0 && !catalog.ValidToSubmitObj())
        {
            ...
            StartCoroutine(controller.canvasMain.GetComponent<Catalog>().TextSleepOff("Please select one of each file"));
        }
        else
        {
